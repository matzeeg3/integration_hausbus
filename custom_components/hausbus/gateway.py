...
class HausbusGateway(IBusDataListener):  # type: ignore[misc]
    ...
    def newDeviceDetected(
        ...
        if device.is_leistungs_regler():
            ...
            nr_schalter = sum(
                1 for instance in channels if isinstance(instance, Schalter)
            )
            if nr_schalter > 6:
        ...
        for channel in channels:
            ...
            if object_id not in self.registered_channels:
                ...
                if (
                    ...
                    and isinstance(channel, Schalter)
                    and "Rote Modul LED" not in channel.getName()
                ):
                ...
                if new_entity is not None:
                    LOGGER.debug(
                        f"new channel {new_entity.__class__.__name__} for {channel}"
                    )
                    channel_list = self.get_channel_list(ObjectId(object_id))
                    channel_list[self.get_channel_id(ObjectId(object_id))] = new_entity
                    asyncio.run_coroutine_threadsafe(
                        self._new_channel_listeners[new_domain](new_entity), self._loop
                    ).result()
                    ...
                    if (
                        isinstance(channel, Taster)
                        and self.get_event_entity(channel.getObjectId()) is None
                    ):
                        ...
                        asyncio.run_coroutine_threadsafe(
                            self._new_channel_listeners["EVENTS"](new_channel),
                            self._loop,
                        ).result()
        ...
        if inputs:
            ...
            LOGGER.debug(
                f"{inputs} inputs angemeldet {device.hass_device_entry_id} deviceId {device_id}"
            )
    ...
    async def async_register_device(
        self, device_id: int, device_info: DeviceInfo, hausBusDevice: HausbusDevice
    ):
